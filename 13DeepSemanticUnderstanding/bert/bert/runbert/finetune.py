...
def finetune():
    ...
    # parser.add_argument("--cuda_devices", type=int, nargs='+', default=None, help="CUDA device ids")
    parser.add_argument("--on_memory", dest="on_memory", action='store_true', help="Loading on memory: true or false")
    parser.add_argument("--no-on_memory", dest="on_memory", action='store_false',
    ...
    args = parser.parse_args()
    cuda_devices = [0]
    train_dataset = "/Bert/merge_dev_label"
    # test_dataset = "/Bert/bert/Test/data/finetune/sts_b.test"
    test_dataset = None
    vocab_path = "/Bert/bert/Test/data/pretrain/vocab"
    output_path = "/root/autodl-tmp/data/output_fin/output.finetune.1"
    pretrained_path = "/root/autodl-tmp/output.1/bertlm.epoch.55.end"

    print("Loading Vocab", vocab_path)
    vocab = WordVocab.load_vocab(vocab_path)
    # with open("/Bert/bert/Test/data/pretrain/vocab.txt", "w") as f:
    #     for line in vocab.itos:
    #         f.write(line + "\n")
    ...
    # 预训练和微调的加载数据的方法不一致，why？
    train_dataset = DownstreamDataset(train_dataset, vocab, seq_len=args.seq_len, encoding=args.encoding,
                                      corpus_lines=args.corpus_lines, on_memory=args.on_memory)

    print("Loading Test Dataset", test_dataset)
    test_dataset = DownstreamDataset(test_dataset, vocab, seq_len=args.seq_len,
                                     encoding=args.encoding, on_memory=args.on_memory,
                                     label_separator=args.label_separator, shuffle=args.shuffle) \
        if test_dataset is not None else None
    ...
    bert = BERT(len(vocab), hidden=args.hidden, n_layers=args.layers, attn_heads=args.attn_heads, dropout=args.dropout)
    if pretrained_path:
        ps_dict = torch.load(pretrained_path)  # 训练的时候说：Total Parameters: 19163863
        bs_dict = bert.state_dict()
        for k in bs_dict.keys():
            k1 = "bert." + k
            if k1 in ps_dict:
        ...
        bert.load_state_dict(bs_dict)
        print("Creating BERTDownstream Trainer")
    # 微调ERTDownstreamTrainer 和预训练用 BERTTrainer, why?
    trainer = BERTDownstreamTrainer(bert, len(vocab), classes_n=args.classes_n, pretrained_model_path="",
                                    ...
                                    with_cuda=args.with_cuda, cuda_devices=cuda_devices, log_freq=args.log_freq,
