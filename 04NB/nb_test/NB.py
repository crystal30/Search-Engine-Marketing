...
WordDic = {}
ClassFeaProb = {}
ClassDefaultProb = {}  # ClassDefaultProb 和 ClassProb的区别
ClassProb = {}
...
def ComputeModel():
    ...
    for classid in ClassFeaDic.keys():
        ...
        newsum = (float)(sum + 1)  # +1 避免分母是0
        #Binary Distribution
        #newsum = (float)(ClassFreq[classid]+2*DefaultFreq)
        for wid in ClassFeaDic[classid].keys():
            ClassFeaProb[classid][wid] = (float)(ClassFeaDic[classid][wid]+DefaultFreq)/newsum  # +DefaultFreq 避免分子为0
...
if __name__ == '__main__':
    # 若运行run.sh的时候，需要把argv 转换成 sys.argv
    argv = ["NB.py", "0", "nb_data.test", "model", "out"]
    if len(argv) < 4:
        print("Usage incorrect!")
    elif argv[1] == '1':
        print("start training:")
        TrainingDataFile = argv[2]
        ModelFile = argv[3]
        LoadData()
        ComputeModel()
        SaveModel()
